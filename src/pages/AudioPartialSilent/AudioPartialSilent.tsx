...
import styles from "./AudioPartialSilent.module.less";
import { selectFiles, selectDir } from "@src/utils/file";
import { AudioSilentPartial } from "@src/apis/audio";
...
export default function AudioPartialSilent() {
	...
	useEffect(() => {
		if (audioLocalPath) {
			...
			setAudioUrl(`http://localhost:8080/${audioLocalPath}`);
			setTimeout(() => {
				setShowAudio(true);
			});
			setTimeout(() => {
	...
	const handleTransformOneClick = async () => {
		...
		if (canTrim) {
			output_file_path = `${outputDir}\\partial_silent_${filenameWithSuffix(
				audioLocalPath
			)}`;
			...
			await AudioSilentPartial(audioLocalPath, output_file_path, extra);
				...
				<div className={styles.mainBox}>
					...
					<div className={styles.operateBox}>
						<div className={styles.sliderBox}>
							<div className={styles.sliderTitle}>选择静音时间段</div>
